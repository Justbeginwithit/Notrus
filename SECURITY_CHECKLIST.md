...
- Routine sync, search, message, and attachment traffic now use short-lived session or delivery tokens plus opaque mailbox handles rather than repeating stable sender, thread, and device identifiers in routine request bodies, in [server.js](server.js), [RelayClient.swift](native/macos/NotrusMac/Sources/RelayClient.swift), [AppModel.swift](native/macos/NotrusMac/Sources/AppModel.swift), [RelayClient.kt](native/android/NotrusAndroid/app/src/main/java/com/notrus/android/relay/RelayClient.kt), and [NotrusViewModel.kt](native/android/NotrusAndroid/app/src/main/java/com/notrus/android/ui/NotrusViewModel.kt).
...
- The relay uses opaque user IDs, no last-seen surface, no typing surface, shorter retention windows for ciphertext artifacts, and HMAC-derived blinded rate-limit keys as documented in [METADATA_POLICY.md](METADATA_POLICY.md).
- Both native clients now expose an optional privacy mode that adds short random delay before routine sync, search, thread creation, and send operations.
- The live metadata proof in [test-metadata-boundary.mjs](scripts/test-metadata-boundary.mjs) plus the narrower transport proof in [test-privacy-routing.mjs](scripts/test-privacy-routing.mjs) verify contact-scoped sync, explicit discovery, stripped standards-thread titles, and opaque routine routing.
...
- run `env NOTRUS_PRIVACY_RELAY_ORIGIN=http://127.0.0.1:3062 npm run test:privacy-routing`
