...
APP_VERSION="${NOTRUS_ANDROID_VERSION:-0.3.4-beta5}"
...
BUILD_COUNTER_FILE="$BUILD_COUNTER_DIR/android-$APP_VERSION.counter"
if [[ -n "${NOTRUS_ANDROID_BUILD_COUNTER:-}" ]]; then
  BUILD_COUNTER="$NOTRUS_ANDROID_BUILD_COUNTER"
else
...
BUILD_ID="${NOTRUS_ANDROID_BUILD_ID:-$APP_VERSION+android.$BUILD_COUNTER.$(date -u +%Y%m%d%H%M%S)}"
...
export ANDROID_SDK_ROOT JAVA_HOME GRADLE_USER_HOME NOTRUS_RELEASE_MODE NOTRUS_ANDROID_VERSION
export NOTRUS_ANDROID_BUILD_COUNTER="$BUILD_COUNTER"
export NOTRUS_ANDROID_BUILD_ID="$BUILD_ID"
