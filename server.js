...
const env = (current, legacy) => process.env[current] ?? process.env[legacy];
const DATA_DIR = env("NOTRUS_DATA_DIR", "AEGIS_DATA_DIR")
  ? path.resolve(env("NOTRUS_DATA_DIR", "AEGIS_DATA_DIR"))
  : path.join(__dirname, "data");
const SECRET_DIR = env("NOTRUS_SECRET_DIR", "AEGIS_SECRET_DIR")
  ? path.resolve(env("NOTRUS_SECRET_DIR", "AEGIS_SECRET_DIR"))
  : path.join(__dirname, ".secrets");
const STORE_PATH = env("NOTRUS_STORE_PATH", "AEGIS_STORE_PATH")
  ? path.resolve(env("NOTRUS_STORE_PATH", "AEGIS_STORE_PATH"))
  : path.join(DATA_DIR, "store.json");
const TRANSPARENCY_KEY_PATH = env("NOTRUS_TRANSPARENCY_KEY_PATH", "AEGIS_TRANSPARENCY_KEY_PATH")
  ? path.resolve(env("NOTRUS_TRANSPARENCY_KEY_PATH", "AEGIS_TRANSPARENCY_KEY_PATH"))
...
const ATTESTATION_ORIGIN = env("NOTRUS_ATTESTATION_ORIGIN", "AEGIS_ATTESTATION_ORIGIN")?.trim() ?? "";
const REQUIRE_ANDROID_ATTESTATION = env("NOTRUS_REQUIRE_ANDROID_ATTESTATION", "AEGIS_REQUIRE_ANDROID_ATTESTATION") === "true";
...
const ACTIVE_PROTOCOL_POLICY = resolveProtocolPolicy(env("NOTRUS_PROTOCOL_POLICY", "AEGIS_PROTOCOL_POLICY"));
...
const MESSAGE_RETENTION_MS = Number(process.env.MESSAGE_RETENTION_MS || 30 * 24 * 60 * 60 * 1000);
const ATTACHMENT_RETENTION_MS = Number(process.env.ATTACHMENT_RETENTION_MS || 30 * 24 * 60 * 60 * 1000);
...
const RATE_LIMITS = {
  ...
  registerPerIp: {
    limit: Number(process.env.RATE_REGISTER_PER_IP || 8),
    windowMs: Number(process.env.RATE_REGISTER_PER_IP_WINDOW_MS || 600_000),
  },
  registerPerInstance: {
    limit: Number(process.env.RATE_REGISTER_PER_INSTANCE || 6),
    windowMs: Number(process.env.RATE_REGISTER_PER_INSTANCE_WINDOW_MS || 600_000),
  },
...
function setApiHeaders(request, response) {
  ...
  response.setHeader(
    ...
    "Content-Type,X-Notrus-Integrity,X-Notrus-Instance-Id,X-Notrus-Pow-Nonce,X-Notrus-Pow-Token,X-Notrus-Device-Id,X-Aegis-Integrity,X-Aegis-Instance-Id,X-Aegis-Pow-Nonce,X-Aegis-Pow-Token,X-Aegis-Device-Id"
  );
...
async function loadOrCreateTransparencySigner() {
  ...
  const envPrivateKeyPem = env("NOTRUS_TRANSPARENCY_PRIVATE_KEY_PEM", "AEGIS_TRANSPARENCY_PRIVATE_KEY_PEM")?.trim();
  if (envPrivateKeyPem) {
...
function getRequestInstanceKey(request) {
  const instanceId = request.headers["x-notrus-instance-id"] ?? request.headers["x-aegis-instance-id"];
...
function requireProofOfWork(request, response, scope, integrityObservation) {
  ...
  const token = request.headers["x-notrus-pow-token"] ?? request.headers["x-aegis-pow-token"];
  const nonce = request.headers["x-notrus-pow-nonce"] ?? request.headers["x-aegis-pow-nonce"];
...
function parseIntegrityReportHeader(request) {
  const encoded = request.headers["x-notrus-integrity"] ?? request.headers["x-aegis-integrity"];
...
function getRequestDeviceId(request) {
  const deviceId = request.headers["x-notrus-device-id"] ?? request.headers["x-aegis-device-id"];
...
function sanitizeAttachmentRecord(attachment, { senderId, threadId }) {
  if (
    ...
    !isNonEmptyString(attachment.senderId, 120) ||
    !isNonEmptyString(attachment.threadId, 160) ||
  ...
  if (attachment.senderId.trim() !== senderId || attachment.threadId.trim() !== threadId) {
    return null;
  }
  ...
  return {
    ...
    senderId: attachment.senderId.trim(),
    sha256: attachment.sha256.trim(),
    threadId: attachment.threadId.trim(),
  };
}
...
function searchDirectoryRecords(query, requestingUserId, { exactUsernameOrInviteOnly = false } = {}) {
  ...
  if (exactUsernameOrInviteOnly) {
    ...
    return Object.values(store.users)
      ...
      .map((user) => publicUserRecord(user));
  }
...
async function handleSync(request, response, url) {
  ...
  sendJson(request, response, 200, {
    ...publicTransparencyState(),
    directoryDiscoveryMode: "username-or-invite",
    deviceEvents: (store.users[userId].deviceEvents ?? []).map((event) => publicDeviceEventRecord(event)),
    devices: (store.users[userId].devices ?? []).map((device) => publicDeviceRecord(device, { currentDeviceId: requestDeviceId })),
    users,
    threads,
  });
}
...
async function handleDirectorySearch(request, response, url) {
  ...
  sendJson(request, response, 200, {
    ...
    query: normalizeDirectoryCode(query) ?? normalizeUsername(query),
  ...
  if (
    !enforceRateLimits(
      ...
      [
        ...
        { config: RATE_LIMITS.reportPerUser, key: reporterId, scope: "report-user" },
      ],
      "Report submission"
    )
  ) {
...
async function handleAccountReset(request, response) {
  ...
  sendJson(request, response, 200, { ok: true, user: publicUserRecord(existing, { viewerUserId: existing.id }) });
}
...
async function handleCreateThread(request, response) {
  ...
  if (!requireProofOfWork(request, response, "create-thread", integrityObservation)) {
    return;
  }
  ...
  const createdBy = isNonEmptyString(body.createdBy, 120) ? body.createdBy.trim() : "";
  ...
  const participantIds = dedupeStringArray(body.participantIds);
  ...
  if (
    !enforceRateLimits(
      ...
      [
        ...
        { config: RATE_LIMITS.threadPerUser, key: createdBy, scope: "thread-user" },
      ],
      "Thread creation"
    )
  ) {
...
async function handlePostMessage(request, response, threadId) {
  pruneExpiredArtifacts();
  const integrityObservation = sanitizeIntegrityObservation(parseIntegrityReportHeader(request));

  if (!requireProofOfWork(request, response, "post-message", integrityObservation)) {
    return;
  }
  ...
  const senderId = isNonEmptyString(body.senderId, 120) ? body.senderId.trim() : "";
  const createdAt = isNonEmptyString(body.createdAt, 60) ? body.createdAt.trim() : "";
  ...
  if (
    !enforceRateLimits(
      ...
      [
        ...
        { config: RATE_LIMITS.messagePerUser, key: senderId, scope: "message-user" },
      ],
      "Message"
    )
  ) {
...
async function handleUploadAttachment(request, response, threadId) {
  pruneExpiredArtifacts();
  const integrityObservation = sanitizeIntegrityObservation(parseIntegrityReportHeader(request));

  if (!requireProofOfWork(request, response, "upload-attachment", integrityObservation)) {
    return;
  }
  ...
  if (
    !enforceRateLimits(
      ...
      [
        ...
        { config: RATE_LIMITS.messagePerUser, key: senderId, scope: "attachment-user" },
      ],
      "Attachment"
    )
  ) {
...
async function handleFetchAttachment(request, response, threadId, attachmentId, url) {
  const thread = store.threads[threadId];
  if (!thread) {
    sendError(request, response, 404, "Thread not found.");
    return;
