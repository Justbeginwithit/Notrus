...
enum NotrusCrypto {
    ...
    static func transferredIdentity(from snapshot: PortableArchiveIdentitySnapshot) throws -> LocalIdentity {
        let signingPrivateKey = P256.Signing.PrivateKey()
        let encryptionPrivateKey = P256.KeyAgreement.PrivateKey()
        let signingPublicJwk = try jwk(from: signingPrivateKey.publicKey.x963Representation)
        let encryptionPublicJwk = try jwk(from: encryptionPrivateKey.publicKey.x963Representation)
        let prekey = try createSignedPrekey(signingKey: .software(signingPrivateKey), userId: snapshot.id)
        ...
        return LocalIdentity(
            ...
            recoveryFingerprint: snapshot.recoveryFingerprint,
            recoveryPublicJwk: snapshot.recoveryPublicJwk,
    ...
    static func sealAttachment(
        ...
        let createdAt = isoNow()
        let iv = randomData(count: 12)
        let padded = try padAttachmentPayload(data: data)
        let aad = Data(
            """
    ...
    static func openAttachment(_ attachment: RelayAttachment, reference: SecureAttachmentReference) throws -> Data {
        ...
        if let unpadded = try? unpadAttachmentPayload(data: plaintext) {
            return unpadded
        }
        return plaintext
    }
    ...
    static func recoveryAuthority(from recoveryRepresentation: String) throws -> (fingerprint: String, publicJwk: JWK) {
        let recoveryKey = try loadSigningPrivateKey(recoveryRepresentation)
        let publicJwk = try jwk(from: recoveryKey.publicKey.x963Representation)
        let fingerprint = try computeFingerprint(for: publicJwk)
    ...
    static func normalizedForRelay(_ request: AccountResetRequest) -> AccountResetRequest {
        AccountResetRequest(
            createdAt: request.createdAt.trimmingCharacters(in: .whitespacesAndNewlines),
            device: request.device,
            displayName: request.displayName.trimmingCharacters(in: .whitespacesAndNewlines),
            encryptionPublicJwk: normalizedJwk(request.encryptionPublicJwk),
            fingerprint: request.fingerprint.trimmingCharacters(in: .whitespacesAndNewlines),
            mlsKeyPackage: request.mlsKeyPackage.map(normalizedMlsKeyPackage),
            prekeyCreatedAt: request.prekeyCreatedAt.trimmingCharacters(in: .whitespacesAndNewlines),
            prekeyFingerprint: request.prekeyFingerprint.trimmingCharacters(in: .whitespacesAndNewlines),
            prekeyPublicJwk: normalizedJwk(request.prekeyPublicJwk),
            prekeySignature: request.prekeySignature.trimmingCharacters(in: .whitespacesAndNewlines),
            recoveryFingerprint: request.recoveryFingerprint.trimmingCharacters(in: .whitespacesAndNewlines),
            recoveryPublicJwk: normalizedJwk(request.recoveryPublicJwk),
            recoverySignature: request.recoverySignature.trimmingCharacters(in: .whitespacesAndNewlines),
            signalBundle: request.signalBundle.map(normalizedSignalBundle),
            signingPublicJwk: normalizedJwk(request.signingPublicJwk),
            userId: request.userId.trimmingCharacters(in: .whitespacesAndNewlines),
            username: request.username.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        )
    }
    ...
    static func attachmentPayloadData(base64Payload: String, padding: String, size: Int) throws -> Data {
    ...
    static func padAttachmentPayload(data: Data) throws -> (serialized: Data, paddingBucket: Int) {
        ...
        var padding = ""
        let target = choosePaddingBucket(try attachmentPayloadData(base64Payload: payload, padding: padding, size: data.count).count)

        while try attachmentPayloadData(base64Payload: payload, padding: padding, size: data.count).count < target {
            padding.append(".")
        }

        while !padding.isEmpty {
            let currentSize = try attachmentPayloadData(base64Payload: payload, padding: padding, size: data.count).count
        ...
        return (
            serialized: try attachmentPayloadData(base64Payload: payload, padding: padding, size: data.count),
            paddingBucket: target
        )
    }

    static func unpadAttachmentPayload(data: Data) throws -> Data {
        guard
            let object = try JSONSerialization.jsonObject(with: data) as? [String: Any],
    ...
    private static func normalizedJwk(_ jwk: JWK) -> JWK {
        JWK(
            crv: jwk.crv.trimmingCharacters(in: .whitespacesAndNewlines),
            kty: jwk.kty.trimmingCharacters(in: .whitespacesAndNewlines),
            x: jwk.x.trimmingCharacters(in: .whitespacesAndNewlines),
            y: jwk.y.trimmingCharacters(in: .whitespacesAndNewlines)
        )
    }

    private static func normalizedMlsKeyPackage(_ bundle: PublicMlsKeyPackage) -> PublicMlsKeyPackage {
        PublicMlsKeyPackage(
            ciphersuite: bundle.ciphersuite.trimmingCharacters(in: .whitespacesAndNewlines),
            keyPackage: bundle.keyPackage.trimmingCharacters(in: .whitespacesAndNewlines)
        )
    }

    private static func normalizedSignalBundle(_ bundle: PublicSignalBundle) -> PublicSignalBundle {
        PublicSignalBundle(
            deviceId: bundle.deviceId,
            identityKey: bundle.identityKey.trimmingCharacters(in: .whitespacesAndNewlines),
            kyberPreKeyId: bundle.kyberPreKeyId,
            kyberPreKeyPublic: bundle.kyberPreKeyPublic.trimmingCharacters(in: .whitespacesAndNewlines),
            kyberPreKeySignature: bundle.kyberPreKeySignature.trimmingCharacters(in: .whitespacesAndNewlines),
            preKeyId: bundle.preKeyId,
            preKeyPublic: bundle.preKeyPublic.trimmingCharacters(in: .whitespacesAndNewlines),
            registrationId: bundle.registrationId,
            signedPreKeyId: bundle.signedPreKeyId,
            signedPreKeyPublic: bundle.signedPreKeyPublic.trimmingCharacters(in: .whitespacesAndNewlines),
            signedPreKeySignature: bundle.signedPreKeySignature.trimmingCharacters(in: .whitespacesAndNewlines)
        )
    }
}
