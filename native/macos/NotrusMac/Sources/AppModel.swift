...
final class AppModel: ObservableObject {
    private static let legacyDefaultsSuite = "com.aegis.mac"
    private static let legacyRelayOriginKey = "AegisMac.relayOrigin"
    private static let legacyWitnessOriginsKey = "AegisMac.witnessOrigins"
    private static let legacyPinnedHeadsKey = "AegisMac.transparencyPins"
    private static let legacyPinnedSignerKeysKey = "AegisMac.transparencySignerPins"
    private static let defaultLocalRelayOrigin = "http://127.0.0.1:3000"
    private static let defaultRemoteRelayOrigin = {
        let configured = (ProcessInfo.processInfo.environment["NOTRUS_DEFAULT_RELAY_ORIGIN"] ?? ProcessInfo.processInfo.environment["AEGIS_DEFAULT_RELAY_ORIGIN"])?
    ...
    private let configuredProtocolPolicy = ProtocolPolicyMode(
        rawValue: ProcessInfo.processInfo.environment["NOTRUS_PROTOCOL_POLICY"] ?? ProcessInfo.processInfo.environment["AEGIS_PROTOCOL_POLICY"] ?? ""
    ) ?? .requireStandards
    ...
    init(
        ...
        self.relayOrigin = Self.bootstrapRelayOrigin(Self.storedString(forKey: "NotrusMac.relayOrigin", legacyKey: Self.legacyRelayOriginKey))
        self.witnessOriginsText = Self.storedString(forKey: witnessOriginsKey, legacyKey: Self.legacyWitnessOriginsKey) ?? ""
        self.transparencyPins = Self.loadPinnedHeads()
        self.transparencySignerPins = Self.loadPinnedSignerKeys()
    }

    var contacts: [RelayUser] {
        users.filter { $0.id != currentIdentity?.id }
    }
    ...
    private func relayClientAndHealth(allowRemoteFallback: Bool = true) async throws -> (RelayClient, RelayHealth) {
        ...
        do {
            return (primaryClient, try await primaryClient.health())
        } catch {
    ...
    private func clearRemoteWorkspace() {
        currentUser = nil
        linkedDeviceEvents = []
        linkedDevices = []
        users = []
    ...
    func resetTransparencyTrust() async {
        ...
        Self.legacyDefaults()?.removeObject(forKey: Self.legacyPinnedHeadsKey)
        Self.legacyDefaults()?.removeObject(forKey: Self.legacyPinnedSignerKeysKey)
        transparency = .empty
    ...
    func sync() async throws {
        ...
        let (client, health) = try await relayClientAndHealth()
        relayProtocolPolicy = health.protocolPolicy
        let payload = try await client.sync(userId: identity.id)
        ...
        transparency = try await verifyTransparency(
            entryCount: payload.entryCount ?? payload.transparencyEntries?.count ?? 0,
            entries: payload.transparencyEntries ?? [],
            expectedHead: payload.transparencyHead,
            expectedSignature: payload.transparencySignature,
            signer: payload.transparencySigner
        )
        ...
        linkedDevices = (payload.devices ?? []).sorted { $0.updatedAt > $1.updatedAt }
        linkedDeviceEvents = (payload.deviceEvents ?? []).sorted { $0.createdAt > $1.createdAt }
        threads = decryptedThreads

        if selectedThreadID == nil {
            selectedThreadID = decryptedThreads.first?.id
        } else if !decryptedThreads.contains(where: { $0.id == selectedThreadID }) {
            selectedThreadID = decryptedThreads.first?.id
        }

        if !transparency.chainValid {
            statusMessage = "Synced \(directory.count) identities and \(decryptedThreads.count) threads. Transparency verification needs attention."
        } else if hasPendingSecurityActions {
            statusMessage = "Synced \(directory.count) identities and \(decryptedThreads.count) threads. Contact security review needs attention."
        } else {
            statusMessage = "Synced \(directory.count) identities and \(decryptedThreads.count) threads from the relay."
        }
    }
    ...
    func searchDirectory() async {
        ...
        do {
            let response = try await currentRelayClient.searchDirectory(userId: identity.id, query: query.lowercased())
    ...
    func importAccount(from url: URL, passphrase: String) async {
        ...
        do {
            ...
            let archive = try await NativeExecution.run {
            ...
            clearRemoteWorkspace()
            try await registerAndSync()
            statusMessage = "Imported \(archive.identity.displayName)'s encrypted recovery archive from \(url.lastPathComponent)."
        } catch {
            if !isUserCancellation(error) {
                errorMessage = error.localizedDescription
            }
    ...
    func createThread() async {
        ...
        do {
            ...
            let participantsById = Dictionary(uniqueKeysWithValues: composeCandidates.map { ($0.id, $0) })
    ...
    func sendMessage() async {
        ...
        do {
            ...
            for draft in pendingAttachments {
                ...
                _ = try await currentRelayClient.uploadAttachment(threadId: thread.id, attachment: sealed.request)
    ...
    func deleteCurrentIdentity() async {
        ...
        do {
            _ = try await deviceSecretStore.reauthenticateInteractively(
                reason: "Confirm local device authentication before deleting the active Notrus profile from this Mac."
            )
            refreshUnlockedDeviceSessionMetadata()
            ...
            if currentIdentity != nil {
                try await registerAndSync()
                statusMessage = "Removed \(identity.displayName)'s local profile and switched to the next account."
            } else {
                statusMessage = "Removed the last local Notrus profile from this Mac."
            }
        } catch {
            errorMessage = error.localizedDescription
        }
    ...
    func rotateCurrentIdentityKeys() async {
        ...
        do {
            ...
            _ = try await currentRelayClient.resetAccount(signedRequest)
            threadRecords = [:]
            clearRemoteWorkspace()
            try persistCurrentIdentity(rotatedIdentity)
            try persistThreadStore()
            try await registerAndSync()
    ...
    private func sendStaticMessage(thread: ConversationThread, identity: LocalIdentity, text: String) async throws {
        ...
        _ = try await currentRelayClient.postMessage(threadId: thread.id, message: outbound)
    ...
    private func sendPairwiseMessage(thread: ConversationThread, identity: LocalIdentity, payload: MessagePayload) async throws {
        ...
        do {
            _ = try await currentRelayClient.postMessage(threadId: thread.id, message: sealed.message)
    ...
    private static func loadPinnedHeads() -> [String: String] {
        guard
            let raw = storedData(forKey: "NotrusMac.transparencyPins", legacyKey: legacyPinnedHeadsKey),
    ...
    private static func loadPinnedSignerKeys() -> [String: String] {
        guard
            let raw = storedData(forKey: "NotrusMac.transparencySignerPins", legacyKey: legacyPinnedSignerKeysKey),
            let decoded = try? JSONDecoder().decode([String: String].self, from: raw)
        else {
            return [:]
        }
    ...
    private static func storedString(forKey key: String, legacyKey: String) -> String? {
        if let value = UserDefaults.standard.string(forKey: key), !value.isEmpty {
            return value
        }
        return legacyDefaults()?.string(forKey: legacyKey)
    }

    private static func storedData(forKey key: String, legacyKey: String) -> Data? {
        if let value = UserDefaults.standard.data(forKey: key) {
            return value
        }
        return legacyDefaults()?.data(forKey: legacyKey)
    }
    ...
    private static func legacyDefaults() -> UserDefaults? {
        UserDefaults(suiteName: legacyDefaultsSuite)
    }
}
