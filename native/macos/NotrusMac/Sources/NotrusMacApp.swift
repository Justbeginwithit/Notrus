            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    if let identity = model.currentIdentity {
                        SidebarIdentityCard(
                            identity: identity,
                            contactsCount: model.contacts.count,
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            ViewThatFits(in: .horizontal) {
                HStack(spacing: 10) {
                    ...
                    MiniStat(
                        label: "Verified",
                        value: "\(model.contactSecurityState.contacts.values.filter { $0.status == .verified }.count)"
                    )
                }
                VStack(spacing: 10) {
                    MiniStat(
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 14) {
                            ...
                            if model.contactSecurityState.contacts.isEmpty {
                                Text("No contacts have been observed on this Mac yet.")
                                    .font(.callout)
                                    .foregroundStyle(.secondary)
                            } else {
                                VStack(spacing: 12) {
                                    ForEach(model.contactSecurityState.contacts.values.sorted(by: { $0.displayName < $1.displayName })) { record in
                                        VStack(alignment: .leading, spacing: 10) {
                                            ViewThatFits(in: .horizontal) {
                                                HStack(alignment: .top, spacing: 12) {
