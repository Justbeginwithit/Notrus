...
struct LocalDeviceInventoryProfile: Hashable {
    let id: String
    let username: String
    let displayName: String
    let createdAt: String
...
struct LocalDeviceInventory: Hashable {
    let vaultStorageMode: String
    let vaultKeyPresent: Bool
    let metadataDirectoryLabel: String
    let appInstanceId: String?
    let deviceId: String?
    let deviceKeyStorageMode: String?
    let profileCount: Int
    let profiles: [LocalDeviceInventoryProfile]

    static let empty = LocalDeviceInventory(
        vaultStorageMode: "unknown",
        vaultKeyPresent: false,
        metadataDirectoryLabel: "NotrusMac/device-metadata",
        appInstanceId: nil,
        deviceId: nil,
        deviceKeyStorageMode: nil,
        profileCount: 0,
        profiles: []
    )
}
...
struct PortableArchiveIdentitySnapshot: Codable, Equatable {
    let id: String
    let username: String
    let displayName: String
    let createdAt: String
    ...
    init(
        ...
        recoveryPublicJwk: JWK,
        recoveryRepresentation: String
    ) {
        self.id = id
        self.username = username
        self.displayName = displayName
        self.createdAt = createdAt
        self.recoveryFingerprint = recoveryFingerprint
        self.recoveryPublicJwk = recoveryPublicJwk
        self.recoveryRepresentation = recoveryRepresentation
    }
...
struct RecoveryTransferArchive: Codable, Equatable {
    let version: Int
    let exportedAt: String
...
struct RelayUser: Codable, Identifiable, Hashable {
    ...
    let contactHandleExpiresAt: String?
    let directoryCode: String?
    let fingerprint: String
    ...
    init(
        ...
        contactHandleExpiresAt: String? = nil,
        ...
        self.contactHandleExpiresAt = contactHandleExpiresAt
...
struct RelayThread: Codable, Identifiable, Hashable {
    ...
    let id: String
    let title: String
    let mailboxHandle: String?
    let mailboxHandleExpiresAt: String?
    let protocolField: String?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case deliveryCapabilityExpiresAt
        case id
        case mailboxHandle
        case mailboxHandleExpiresAt
        case title
        case protocolField = "protocol"
        case groupState
...
struct RelayTransparencySnapshot: Codable {
    let entryCount: Int?
    let relayTime: String?
...
struct RelaySecurityDevicesResponse: Codable {
    let deviceEvents: [RelayDeviceEvent]
    let devices: [RelayLinkedDevice]
}

struct RelaySession: Codable, Hashable {
...
struct OutboundMessage: Codable {
    ...
    init(
        ...
        self.transportPadding = transportPadding
        self.wireMessage = wireMessage
    }
}

private func nextTransportBucket(for visibleSize: Int) -> Int {
    let buckets = [512, 1024, 2048, 4096, 8192, 16384, 32768, 65536]
    return buckets.first(where: { visibleSize <= $0 }) ?? max(visibleSize, 65536)
}

private func transportPaddingFill(visibleSize: Int) -> String? {
    let target = nextTransportBucket(for: visibleSize)
    let deficit = max(0, target - visibleSize)
    guard deficit > 32 else {
        return nil
    }

    let seed = UUID().uuidString.replacingOccurrences(of: "-", with: "")
    let repeated = String(repeating: seed, count: Int(ceil(Double(deficit) / Double(seed.count))))
    return String(repeated.prefix(deficit))
}

extension OutboundMessage {
    func relayTransportForm() -> OutboundMessage {
        let visibleSize =
            (ciphertext?.count ?? 0) +
            (wireMessage?.count ?? 0) +
            (signature?.count ?? 0) +
        ...
        return OutboundMessage(
            ...
            paddingBucket: paddingBucket ?? nextTransportBucket(for: max(visibleSize, 1)),
            protocolField: protocolField,
            ratchetPublicJwk: ratchetPublicJwk,
            senderId: nil,
            signature: signature,
            threadId: nil,
